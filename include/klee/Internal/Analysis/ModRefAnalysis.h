...
public:
  ...
  typedef std::map<llvm::Function *, SVF::PointsTo> ModPtsMap;
  typedef std::map<llvm::Function *, InstructionSet> ModSetMap;

  typedef std::map<llvm::Function *, SVF::PointsTo> RefPtsMap;

  typedef std::map<std::pair<llvm::Function *, SVF::NodeID>, InstructionSet>
      ObjToStoreMap;
  typedef std::map<std::pair<llvm::Function *, SVF::NodeID>, InstructionSet>
      ObjToLoadMap;
  typedef std::map<SVF::NodeID, InstructionSet> ObjToOverridingStoreMap;
...
private:
  ...
  AllocSite getAllocSite(SVF::NodeID);

  bool hasReturnValue(llvm::Function *f);

  llvm::MemoryLocation getLoadLocation(llvm::LoadInst *inst);

  llvm::MemoryLocation getStoreLocation(llvm::StoreInst *inst);
