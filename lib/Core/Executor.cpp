...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
  ...
  Solver *solver = constructSolverChain(
      ...
      concretizationManager.get(), addressManager);
...
void Executor::addConstraint(ExecutionState &state, ref<Expr> condition) {
  ...
  if (concretizationManager->contains(state.constraints, condition)) {
    Assignment oldAssignment = state.constraints.getConcretization();
    ...
    for (auto it : newAssignment.bindings) {
  ...
  if (unbound) {
    if (incomplete) {
      ...
      IDType idLazyInitialization =
          lazyInitializeObject(*unbound, base, target, baseTargetType, sizeExpr);
...
void Executor::getConstraintLog(const ExecutionState &state, std::string &res,
  ...
  case SMTLIB2: {
    std::string Str;
    llvm::raw_string_ostream info(Str);
    ExprSMTLIBPrinter printer(concretizationManager.get());
    ...
    for (const auto &innerTypeOffset : symbolicType->getInnerTypes()) {
      ...
      for (const auto &offset : innerTypeOffset.second) {
        ...
        if (state.resolveOnSymbolics(constantAddress, idResult)) {
          const MemoryObject *mo =
              state.addressSpace.findObject(idResult).first;
