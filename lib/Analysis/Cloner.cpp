...
Cloner::Cloner(llvm::Module *module, ReachabilityAnalysis *ra,
               raw_ostream &debugs)
    : module(module), ra(ra), debugs(debugs) {}
...
void Cloner::clone(Function *f, uint32_t sliceId) {
  ...
  debugs << f->getName() << ": " << reachable.size()
         << " reachable functions\n";

  for (set<Function *>::iterator j = reachable.begin(); j != reachable.end();
       j++) {
    Function *f = *j;
    if (f->isDeclaration()) {
      continue;
...
void Cloner::cloneFunction(Function *f, uint32_t sliceId) {
  ...
  Function *cloned = CloneFunction(f, *v2vmap);

  /* set function name */
  string clonedName =
      f->getName().str() + string("_clone_") + to_string(sliceId);
  cloned->setName(StringRef(clonedName));

  /* update map */
  SliceInfo sliceInfo = {.f = cloned, .isSliced = false, .v2vmap = v2vmap};
...
Cloner::ValueTranslationMap *
Cloner::buildReversedMap(ValueToValueMapTy *v2vmap) {
  ValueTranslationMap *map = new ValueTranslationMap();
  for (ValueToValueMapTy::iterator i = v2vmap->begin(); i != v2vmap->end();
       i++) {
    /* TODO: should be const Value... */
    Value *value = (Value *)(i->first);
    WeakTrackingVH &wvh = i->second;
...
Cloner::SliceInfo *Cloner::getSliceInfo(llvm::Function *function,
                                        uint32_t sliceId) {
...
Cloner::~Cloner() {
  for (FunctionMap::iterator i = functionMap.begin(); i != functionMap.end();
       i++) {
    SliceMap &sliceMap = i->second;
