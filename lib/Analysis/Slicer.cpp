...
//     llvm::cl::desc("<input file>"), llvm::cl::init("test"),
//     llvm::cl::cat(SlicingOpts));
std::string llvmfile = "test";

// llvm::cl::opt<std::string> slicing_criterion("c", llvm::cl::Required,
//     llvm::cl::desc("Slice with respect to the call-sites of a given
//     function\n"
//                    "i. e.: '-c foo' or '-c __assert_fail'. Special value is a
//                    'ret'\n" "in which case the slice is taken with respect to
//                    the return value\n" "of the main() function. You can use
//                    comma separated list of more\n" "function calls, e.g. -c
...
llvm::cl::opt<uint64_t> pta_field_sensitivie(
    ...
    llvm::cl::value_desc("N"), llvm::cl::init(dg::Offset::getUnknown().offset),
    llvm::cl::cat(SlicingOpts));

llvm::cl::opt<bool> rd_strong_update_unknown(
    "rd-strong-update-unknown",
    llvm::cl::desc(
        "Let reaching defintions analysis do strong updates on memory defined\n"
...
llvm::cl::opt<bool> undefined_are_pure(
    "undefined-are-pure",
...
//  functionality
/// --------------------------------------------------------------------
klee::Slicer::Slicer(llvm::Module *mod, uint32_t o, std::string entryFunction,
                     std::vector<std::string> criterions,
                     dg::LLVMPointerAnalysis *llvmpta, Cloner *cloner)
    : M(mod), opts(o), entryFunction(entryFunction), criterions(criterions),
      PTA(llvmpta), slicer(mod, SlicerOptions{}) {
...
bool klee::Slicer::buildDG() {
  ...
  // TODO: do we supposed to build DG twice?
  slicer.buildDG();

  dg.build(M, PTA, nullptr, M->getFunction(entryFunction));
...
bool klee::Slicer::mark() {
  ...
  // unneeded one
  const char *sc[] = {
      "__VERIFIER_assume", "__VERIFIER_exit",
      //"klee_assume",
      "exit",
...
void klee::Slicer::computeEdges() {
  ...
  // dg::LLVMDefUseAnalysis DUA(&dg, RD.get(), PTA, undefined_are_pure);
  // tm.start();
...
void klee::Slicer::remove_unused_from_module_rec() {
  bool fixpoint;
...
bool klee::Slicer::remove_unused_from_module() {
...
void klee::Slicer::make_declarations_external() {
  using namespace llvm;
...
klee::Slicer::~Slicer() { /*clearConstructedFunctions();*/ }
