...
void SliceGenerator::generate() {
  ...
  /* notes:
     - UNKNOWN_OFFSET: field sensitive (not sure if this flag changes
     anything...)
     - main: we need the nodes of the whole program
  */
  llvmpta = new dg::DGLLVMPointerAnalysis(module, "main");
  llvmpta->getPS()->setEntry(new dg::pta::PointerSubgraph());
  ...
  for (ModRefAnalysis::SideEffects::iterator i = sideEffects.begin();
       i != sideEffects.end(); i++) {
    generateSlice(i->getFunction(), i->id, i->type);
  }
}

void SliceGenerator::generateSlice(Function *f, uint32_t sliceId,
  ...
  case ModRefAnalysis::Modifier:
    ...
    for (std::set<std::string>::iterator i = fnames.begin(); i != fnames.end();
         i++) {
      std::string fname = *i;
  ...
  klee::Slicer slicer(module, 0, entryName, criterions, llvmpta, cloner);
...
void SliceGenerator::dumpSlice(Function *f, uint32_t sliceId,
  ...
  for (set<Function *>::iterator i = functions.begin(); i != functions.end();
       i++) {
    Function *g = *i;
    if (g->isDeclaration()) {
      continue;
