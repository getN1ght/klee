...
#include <llvm-14/llvm/Analysis/MemoryLocation.h>
#include <map>
#include <set>
#include <vector>
...
ModRefAnalysis::ModRefAnalysis(llvm::Module *module, ReachabilityAnalysis *ra,
                               AAPass *aa, string entry, vector<string> targets,
                               llvm::raw_ostream &debugs)
    : module(module), ra(ra), aa(aa), entry(entry), targets(targets),
      debugs(debugs) {}

Function *ModRefAnalysis::getEntry() { return entryFunction; }

vector<Function *> ModRefAnalysis::getTargets() { return targetFunctions; }

void ModRefAnalysis::run() {
  /* validation */
  entryFunction = module->getFunction(entry);
  if (!entryFunction) {
    errs() << "entry function '" << entry
...
void ModRefAnalysis::collectModInfo(Function *entry) {
  ...
  for (set<Function *>::iterator i = reachable.begin(); i != reachable.end();
       i++) {
    Function *f = *i;
    if (f->isDeclaration()) {
      continue;
...
void ModRefAnalysis::addStore(Function *f, Instruction *store) {
  llvm::MemoryLocation storeLocation =
      getStoreLocation(dyn_cast<StoreInst>(store));
  SVF::NodeID id =
      aa->getPTA()->getPAG()->getValueNode(SVFValueFrom(storeLocation.Ptr));
  const SVF::PointsTo &pts = aa->getPTA()->getPts(id);

  SVF::PointsTo &modPts = modPtsMap[f];
  ...
  for (SVF::PointsTo::iterator i = pts.begin(); i != pts.end(); ++i) {
    ...
    pair<Function *, SVF::NodeID> k = make_pair(f, nodeId);
    objToStoreMap[k].insert(store);
    modPts.set(nodeId);
  }
}

bool ModRefAnalysis::canIgnoreStackObject(Function *f, const Value *value) {
  bool result;
  ...
  Function *allocatingFunction =
      dyn_cast<Function>(alloca->getParent()->getParent());
...
void ModRefAnalysis::collectRefInfo(Function *entry) {
  ...
  for (set<Instruction *>::iterator i = reachable.begin(); i != reachable.end();
       i++) {
    Instruction *inst = *i;
...
void ModRefAnalysis::addLoad(Function *f, Instruction *load) {
  MemoryLocation loadLocation = getLoadLocation(dyn_cast<LoadInst>(load));
  SVF::NodeID id =
      aa->getPTA()->getPAG()->getValueNode(SVFValueFrom(loadLocation.Ptr));
  const SVF::PointsTo &pts = aa->getPTA()->getPts(id);

  SVF::PointsTo &refPts = refPtsMap[f];
  refPts |= pts;
...
void ModRefAnalysis::addOverridingStore(Instruction *store) {
  MemoryLocation storeLocation = getStoreLocation(dyn_cast<StoreInst>(store));
  SVF::NodeID id =
      aa->getPTA()->getPAG()->getValueNode(SVFValueFrom(storeLocation.Ptr));
  const SVF::PointsTo &pts = aa->getPTA()->getPts(id);
...
void ModRefAnalysis::computeModRefInfo() {
  for (ModPtsMap::iterator i = modPtsMap.begin(); i != modPtsMap.end(); i++) {
    Function *f = i->first;
    SVF::PointsTo &modPts = i->second;

    /* get the corresponding ref-set */
    SVF::PointsTo &refPts = refPtsMap[f];
    /* compute the intersection */
    SVF::PointsTo pts = modPts & refPts;
    /* get the corresponding modifies-set */
    InstructionSet &modSet = modSetMap[f];

    for (SVF::PointsTo::iterator ni = pts.begin(); ni != pts.end(); ++ni) {
      SVF::NodeID nodeId = *ni;
      ...
      overridingStores.insert(localOverridingStores.begin(),
                              localOverridingStores.end());
    }
  }
}
...
void ModRefAnalysis::computeModInfoToStoreMap() {
  ...
  for (vector<Function *>::iterator i = targetFunctions.begin();
       i != targetFunctions.end(); i++) {
    Function *f = *i;
    InstructionSet &modSet = modSetMap[f];
    ...
    if (hasReturnValue(f)) {
      ...
      SideEffect sideEffect = {
          .type = ReturnValue, .id = retSliceId, .info = {.f = f}};
    ...
    for (InstructionSet::iterator i = modSet.begin(); i != modSet.end(); i++) {
      ...
      for (SVF::PointsTo::iterator ni = pts.begin(); ni != pts.end(); ++ni) {
        ...
        if (modInfoToIdMap.find(modInfo) == modInfoToIdMap.end()) {
          ...
          SideEffect sideEffect = {
              .type = Modifier, .id = modSliceId, .info = {.modInfo = modInfo}};
          sideEffects.push_back(sideEffect);
        }
      }
    }
...
ModRefAnalysis::AllocSite ModRefAnalysis::getAllocSite(SVF::NodeID nodeId) {
  SVF::PAGNode *pagNode = aa->getPTA()->getPAG()->getGNode(nodeId);
  SVF::ObjVar *obj = dyn_cast<SVF::ObjVar>(pagNode);
  assert(obj);

  /* get allocation site value */
  const SVF::MemObj *mo = obj->getMemObj();
  const Value *allocSite = LLVMValueFrom(mo->getValue());

  /* get offset in bytes */
  uint64_t offset = 0;
  if (obj->getNodeKind() == SVF::PAGNode::GepObjNode) {
    SVF::GepObjVar *gepObj = dyn_cast<SVF::GepObjVar>(obj);
    // TODO: not sure?
    offset = gepObj->getConstantFieldIdx();
  }
...
MemoryLocation ModRefAnalysis::getLoadLocation(LoadInst *inst) {
  // Value *addr = inst->getPointerOperand();
  return llvm::MemoryLocation::get(inst);
}

MemoryLocation ModRefAnalysis::getStoreLocation(StoreInst *inst) {
  // Value *addr = inst->getPointerOperand();
  return llvm::MemoryLocation::get(inst);
}

/* TODO: validate that a load can't have two ModInfo's with the same allocation
 * site */
void ModRefAnalysis::getApproximateModInfos(Instruction *inst, AllocSite hint,
                                            set<ModInfo> &result) {
  ...
  for (set<ModInfo>::iterator i = modifiers.begin(); i != modifiers.end();
       i++) {
    ModInfo modInfo = *i;
...
void ModRefAnalysis::dumpLoadToModInfoMap() {
  ...
  for (LoadToModInfoMap::iterator i = loadToModInfoMap.begin();
       i != loadToModInfoMap.end(); i++) {
    Instruction *load = i->first;
    set<ModInfo> &modInfos = i->second;

    dumpInst(load);
    for (set<ModInfo>::iterator j = modInfos.begin(); j != modInfos.end();
         j++) {
...
void ModRefAnalysis::dumpModInfoToStoreMap() {
  ...
  for (ModInfoToStoreMap::iterator i = modInfoToStoreMap.begin();
...
void ModRefAnalysis::dumpModInfoToIdMap() {
  ...
  for (ModInfoToIdMap::iterator i = modInfoToIdMap.begin();
       i != modInfoToIdMap.end(); i++) {
    const ModInfo &modInfo = i->first;
...
void ModRefAnalysis::dumpOverridingStores() {
  ...
  for (InstructionSet::iterator j = overridingStores.begin();
       j != overridingStores.end(); j++) {
    Instruction *inst = *j;
    dumpInst(inst);
  }
  debugs << "\n";
