...
public:
  ...
  ConstraintQuery withFalse() const {
    if (constraints.v.empty())
      return *this;
    ConstraintFrames newFrames;
    constraints.butLast(newFrames);
    return ConstraintQuery(newFrames);
  }
...
public:
  ...
  Z3SolverEnv with(const std::vector<const Array *> &objects) const;
};
...
Z3SolverEnv Z3SolverEnv::with(const std::vector<const Array *> &o) const {
  auto result = *this;
  result.objects = o;
  for (size_t i = 0; i < objects.frame_sizes.size(); i++)
    result.objects.push();
  return result;
}
...
bool Z3SolverImpl::computeValue(const ConstraintQuery &query, Z3SolverEnv &env,
  ...
  // for them.
  findSymbolicObjects(query.getOriginalQueryExpr(), objects);
  Z3SolverEnv env1 = env.with(objects);
  if (!computeInitialValues(query.withFalse(), env1, values, hasSolution))
    return false;
...
public:
  ...
  void recreateIfBetter(const ConstraintQuery &query,
...
void Z3IncNativeSolver::recreateIfBetter(const ConstraintQuery &query,
                                         ConstraintDistance &delta) {
  if (!nativeSolver || delta.isOnlyPush() ||
      query.size() >= delta.toPush.size())
    return;
...
private:
  ...
  std::vector<std::unique_ptr<Z3IncNativeSolver>> recentlyUsed;
  std::vector<std::unique_ptr<Z3IncNativeSolver>> recycledSolvers;
...
public:
  ...
  void deinitNativeZ3(Z3_solver theSolver) override {
    ...
    recentlyUsed.push_back(std::move(currentSolver));
  }
...
void Z3TreeSolverImpl::reuseOrCreateZ3(const ConstraintQuery &query,
  ...
  auto min_it = recycledSolvers.end();
  for (auto it = recycledSolvers.begin(), ite = min_it; it != ite; it++) {
  ...
  if (min_it == recycledSolvers.end()) {
    currentSolver =
        std::make_unique<Z3IncNativeSolver>(builder->ctx, solverParameters);
  } else {
    currentSolver = std::move(*min_it);
    recycledSolvers.erase(min_it);
  }
  delta = min_delta;
}
...
void Z3TreeSolverImpl::findSuitableSolver(const ConstraintQuery &query,
  ...
  auto min_it = recentlyUsed.end();
  for (auto it = recentlyUsed.begin(), ite = min_it; it != ite; it++) {
  ...
  auto min_it2 = recycledSolvers.end();
  for (auto it = recycledSolvers.begin(), ite = min_it2; it != ite; it++) {
    (*it)->distance(query, delta);
    auto distance = delta.getDistance();
    if (distance < min_distance) {
      min_delta = delta;
  ...
  assert(min_it != recentlyUsed.end() || min_it2 != recycledSolvers.end());
  delta = min_delta;
  if (min_it2 == recycledSolvers.end()) {
    currentSolver = std::move(*min_it);
    recentlyUsed.erase(min_it);
  } else {
    currentSolver = std::move(*min_it2);
    recycledSolvers.erase(min_it2);
  }
}
...
void Z3TreeSolverImpl::findSuitableSolverLRU(const ConstraintQuery &query,
  ...
  currentSolver = std::move(recentlyUsed.back());
...
ConstraintQuery Z3TreeSolverImpl::prepare(const Query &q) {
  ...
  currentSolver->recreateIfBetter(query, delta);
  assert(currentSolver->isConsistent());
...
bool Z3TreeSolverImpl::computeInitialValues(
    ...
    std::vector<SparseStorage<unsigned char>> &values, bool &hasSolution) {
  auto q = prepare(query);
  auto env = currentSolver->env.with(objects);
  return Z3SolverImpl::computeInitialValues(q, env, values, hasSolution);
}
