...
public:
  ...
  std::unordered_map<const Array *, std::map<uint64_t, Z3ASTHandleLIA>>
      optimizedReads;

  // Map arrays to offsets that should not be optimized
  std::unordered_map<const Array *, std::unordered_set<uint64_t>>
      notOptimizedReads;
