...
bool ConcretizingSolver::relaxSymcreteConstraints(const Query &query,
  ...
  assignment = query.constraints.getConcretization();
  ...
  for (unsigned idx = 0; idx < brokenSymcreteArrays.size(); ++idx) {
    if (isa<SymbolicSizeSource>(brokenSymcreteArrays[idx]->source)) {      
      ...
      if (ref<SymbolicAllocationSource> allocSource =
        ...
        void *address = addressGenerator->allocate(dependentAddressArray, newSize);
        assert(address);
...
bool ConcretizingSolver::computeValidity(const Query &query,
  ...
  if (!trueInvalid) {
    concretizationManager->add(query, falseResponseAssignment);
    ...
    if (!isa<ValidResponse>(solverResponse)) {
      concretizationManager->add(query.negateExpr(), trueResponseAssignment);
      falseInvalid = false;
    }
...
bool ConcretizingSolver::check(const Query &query, ref<SolverResponse> &result) {
  if (!query.containsSymcretes()) {
...
bool ConcretizingSolver::computeTruth(const Query &query, bool &isValid) {
  ...
  if (ref<ConstantExpr> CE = dyn_cast<ConstantExpr>(
          concretizationManager->simplifyExprWithSymcretes(query.constraints, query.expr))) {
    isValid = CE->isTrue(); 
  } else {
    auto concretizedQuery = constructConcretizedQuery(query, assign);
...
Solver *createConcretizingSolver(Solver *s, ConcretizationManager *concretizationManager, AddressGenerator *addressGenerator) {
  return new Solver(new ConcretizingSolver(s, concretizationManager, addressGenerator));
}
}
