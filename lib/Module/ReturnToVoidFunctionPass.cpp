...
bool klee::ReturnToVoidFunctionPass::runOnFunction(Function &f,
  ...
  bool changed = false;
  for (std::vector<Interpreter::SkippedFunctionOption>::const_iterator i =
           skippedFunctions.begin();
       i != skippedFunctions.end(); i++) {
...
Function *
klee::ReturnToVoidFunctionPass::createWrapperFunction(Function &f,
                                                      Module &module) {
  // create new function parameters: *return_var + original function's
  // parameters
  vector<Type *> paramTypes;
  Type *returnType = f.getReturnType();
  paramTypes.push_back(PointerType::get(returnType, 0));
  paramTypes.insert(paramTypes.end(), f.getFunctionType()->param_begin(),
                    f.getFunctionType()->param_end());

  // create new void function
  FunctionType *newFunctionType = FunctionType::get(
      Type::getVoidTy(f.getContext()), makeArrayRef(paramTypes), f.isVarArg());
  string wrappedName = string("__wrap_") + f.getName().str();
  Function *wrapper = cast<Function>(
      module.getOrInsertFunction(wrappedName, newFunctionType).getCallee());
  ...
  BasicBlock *block = BasicBlock::Create(f.getContext(), "entry", wrapper);
...
/// The replacement will occur at all call sites only if the user has not
/// specified a given line in the '-skip-functions' options
void klee::ReturnToVoidFunctionPass::replaceCalls(
    Function *f, Function *wrapper, const vector<unsigned int> &lines) {
  ...
  for (auto ui = f->use_begin(), ue = f->use_end(); ui != ue; ui++) {
    if (Instruction *inst = dyn_cast<Instruction>(*ui)) {
      ...
      if (!lines.empty()) {
        if (auto *N =
                dyn_cast_or_null<llvm::DILocation>(inst->getMetadata("dbg"))) {

          if (find(lines.begin(), lines.end(), N->getLine()) == lines.end()) {
            continue;
          }
        }
      }
...
/// If the original return value was used in a StoreInst, we use directly such
...
void klee::ReturnToVoidFunctionPass::replaceCall(CallInst *origCallInst,
  ...
  for (unsigned int i = 0; i < origCallInst->getNumOperands(); i++) {
  ...
  if (prevStoreInst) {
    ...
    if (hasPhiUse) {
      ...
      Value *load = builder.CreateLoad(allocaInst->getType(), allocaInst);
      origCallInst->replaceAllUsesWith(load);
    } else {
      while (origCallInst->getNumUses() > 0) {
        llvm::Instruction *II =
            cast<llvm::Instruction>(*origCallInst->use_begin());
        IRBuilder<> builder_use(II);
        Value *load = builder_use.CreateLoad(allocaInst->getType(), allocaInst);
