...
ref<Expr> ConstraintSet::simplifyExprWithSymcretes(ref<Expr> expr) const {
  ConstraintSet newCS = concretization.createConstraintsFromAssignment();
  ConstraintManager cm(newCS);
  for (ref<Expr> constraint : constraints) {
    cm.addConstraint(concretization.evaluate(constraint));
  }
  return ConstraintManager::simplifyExpr(newCS, expr);
}
